...
'enviar al cielo o al infierno
If UserList(UserIndex).Stats.ELV < 13 Then
Call WarpUserChar(UserIndex, Mapnew.Map, Mapnew.X, Mapnew.Y, True)
ElseIf Criminal(UserIndex) Then
Call WarpUserChar(UserIndex, Infierno.Map, Infierno.X, Infierno.Y, True)
SendData ToIndex, UserIndex, 0, "||Estas en el Infierno, pero tienes otra oportunidad para regresar a la tierra" & FONTTYPE_INFO
Else
Call WarpUserChar(UserIndex, Cielo.Map, Cielo.X, Cielo.Y, True)
SendData ToIndex, UserIndex, 0, "||Estas en el cielo, Tienes de nuevo una oportunidad para regresar a la tierra" & FONTTYPE_INFO
End If
