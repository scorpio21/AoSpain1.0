...
If UserList(UserIndex).Stats.UserSkills(Navegacion) / ModNave < Barco.MinSkill Then
    Call SendData(ToIndex, UserIndex, 0, "||No tienes suficientes conocimientos." & FONTTYPE_INFO)
...
If UserList(UserIndex).Flags.TargetObjInvIndex > 0 Then
   ...
   Else
        Call SendData(ToIndex, UserIndex, 0, "||No tienes conocimientos de mineria." & FONTTYPE_INFO)
   End If
