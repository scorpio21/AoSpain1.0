...
frmMain.CantUsuarios.Caption = "Users Online: " & NumUsers + especial
...
If UserList(UserIndex).Stats.MinSta < UserList(UserIndex).Stats.MaxSta Then
   ...
   Else
       ...
       If UserList(UserIndex).Stats.MinSta > UserList(UserIndex).Stats.MaxSta Then UserList(UserIndex).Stats.MinSta = UserList(UserIndex).Stats.MaxSta
           Call SendData(ToIndex, UserIndex, 0, "||Te aumenta la Energia." & FONTTYPE_INFO)
           EnviarStats = True
       End If
End If
...
Else
  Call SendData(ToIndex, UserIndex, 0, "||Envenenado, Vas a morir." & FONTTYPE_VENENO)
