...
If NumUsers > DayStats.MaxUsuarios Then DayStats.MaxUsuarios = NumUsers + especial

If NumUsers > recordusuarios Then
    Call SendData(ToAll, 0, 0, "||Record de usuarios conectados simultaniamente." & "Hay " & NumUsers + especial & " usuarios." & FONTTYPE_INFO)
    recordusuarios = NumUsers + especial
...
If UserList(UserIndex).Flags.UserLogged Then UserList(UserIndex).Counters.IdleCount = 0
   ...
   If Not UserList(UserIndex).Flags.UserLogged Then
      ...
      If PassCliente <> "orophin" Then
                    ...
                    Call LogHackAttemp("IP:" & frmMain.Socket2(UserIndex).PeerAddress & " Ip del LAMER.. Cliente NO OFICIAL.")
                    Call CloseSocket(UserIndex)
                    Exit Sub
                End If
                rdata = Right$(rdata, Len(rdata) - 6)
                ...
                Else
                     Call SendData(ToIndex, UserIndex, 0, "ERRAtento si quieres jugar en AOSPain, tienes que bajarte el cliente Propio de AOSpain en www.aospain.com, y podras disfrutar de nuestros mas de 250 mapas, 1000 graficos nuevos, caballos, unicornios, torneos automaticos, sistema antimacros y antichetos, 9 razas y 24 profesiones te esperan en nuestras tierras, entra ya en www.aospain.com")
                     'Call CloseSocket(UserIndex)
                     Exit Sub
                End If
                Exit Sub
...
Select Case UCase$(rdata)
    ...
    Case "AT"
        If UserList(UserIndex).Flags.Muerto = 1 Then
            Call SendData(ToIndex, UserIndex, 0, "||¡¡No puedes atacar estas muerto!!. " & FONTTYPE_INFO)
            Exit Sub
        End If
        ...
        Else
            If UserList(UserIndex).Invent.WeaponEqpObjIndex > 0 Then
                If ObjData(UserList(UserIndex).Invent.WeaponEqpObjIndex).proyectil = 1 Then
                            Call SendData(ToIndex, UserIndex, 0, "||No puedes usar asi esta arma." & FONTTYPE_INFO)
                            Exit Sub
                End If
            End If
    ...
    Case "/ONLINE"
      ...
      ' Next LoopC
       ...
       ' Call SendData(ToIndex, UserIndex, 0, "||" & tStr & FONTTYPE_INFO)
        ...
        Call SendData(ToIndex, UserIndex, 0, "||Número de usuarios: " & NumUsers + especial & FONTTYPE_INFO)
        Exit Sub
    Case "/ONLINEZZ"
        For LoopC = 1 To LastUser
            If (UserList(LoopC).Name <> "") Then
    ...
    Case "/ENTRENAR"
     ...
     Case "/QUEST"
        ...
        'Se asegura que el target es un npc
        If UserList(UserIndex).Flags.TargetNpc = 0 Then
              Call SendData(ToIndex, UserIndex, 0, "||Selecciona al Personaje." & FONTTYPE_INFO)
              Exit Sub
        End If
        If Distancia(Npclist(UserList(UserIndex).Flags.TargetNpc).Pos, UserList(UserIndex).Pos) > 10 Then
                  Call SendData(ToIndex, UserIndex, 0, "||Estas demasiado lejos." & FONTTYPE_INFO)
    ...
    Case "/SALIRCLAN"
     ...
     Case "/EST"
        ...
        Exit Sub
    Case "/ESTADISTICAS"
    On Error GoTo errhandler_est 'ULISES: ojo peta, ver user en log
...
errhandler_est:
    Call LogError("ULISES: Peta con User " & UserList(UserIndex).Name)
        Exit Sub
    ...
    Case "ESKI"
        Call EnviarSkills(UserIndex)
        Exit Sub
            'ULISES: ELIMINAR HECHIZO SELECCIONADO
    Case "/ELIMINAR"
    'Es muy importante el control de errores este saca al server del proceso
On Error GoTo errhandler
            Const BORRAR = 0
            Dim SELECCION As Integer
            SELECCION = UserList(UserIndex).Flags.Hechizo
        If SELECCION = 0 Then
        Call SendData(ToIndex, UserIndex, 0, "||¡¡Selecciona el Hechizo y dale a lanzar!!" & FONTTYPE_FIGHT)
           End If
        UserList(UserIndex).Stats.UserHechizos(SELECCION) = BORRAR
        Call UpdateUserHechizos(False, UserIndex, CByte(SELECCION))
        Call SendData(ToIndex, UserIndex, 0, "||¡¡El Hechizo Fue Eliminado!!" & FONTTYPE_FIGHT)

errhandler:
        Exit Sub
         
      
      'ULISES: MUEVE HECHIZO SELECCIONADO
    Case "/MOVER"
    'Es muy importante el control de errores este saca al server del proceso
On Error GoTo errhandler2
            
            Const MUEVE = 0
            Dim MueveEso As Integer
            Dim Object As Integer
            Dim IndexHECHIZO As Integer
            Dim SLOTS As Integer
            MueveEso = UserList(UserIndex).Flags.Hechizo
            Dim OBJETOS As Integer
        If MueveEso = 0 Then
         Call SendData(ToIndex, UserIndex, 0, "||¡¡Selecciona el Hechizo y dale a lanzar!!" & FONTTYPE_FIGHT)
        End If
        IndexHECHIZO = UserList(UserIndex).Stats.UserHechizos(MueveEso)
        For Object = 1 To NumObjDatas
        
          If (IndexHECHIZO = ObjData(Object).HechizoIndex) And (IndexHECHIZO <> 0) Then
           OBJETOS = Object
           Exit For
          Else: OBJETOS = 0
         End If
    ...
    'Busca un slot vacio
 If SLOTS < MAX_INVENTORY_SLOTS Then
   SLOTS = 1
   Do Until UserList(UserIndex).Invent.Object(SLOTS).ObjIndex = 0
       SLOTS = SLOTS + 1
       If SLOTS > MAX_INVENTORY_SLOTS Then
           Call SendData(ToIndex, UserIndex, 0, "||No pudes cargar mas objetos." & FONTTYPE_FIGHT)
           GoTo errhandler2
       End If
   Loop
...
End If
    ...
    UserList(UserIndex).Invent.Object(SLOTS).ObjIndex = OBJETOS 'ULISES: Pone el numero del objeto
    UserList(UserIndex).Invent.Object(SLOTS).Amount = 1 'ULISES: Pone a 1 la Cantidad de Objetos
    UserList(UserIndex).Stats.UserHechizos(MueveEso) = BORRAR
    Call UpdateUserHechizos(False, UserIndex, CByte(MueveEso))
    Call SendData(ToIndex, UserIndex, 0, "||¡¡El Hechizo Fue Movido!!" & FONTTYPE_FIGHT)
    Call UpdateUserInv(True, UserIndex, 0)

errhandler2:

Call LogError("Error en /MOVER," & Err.Description)
Exit Sub
...
'            Call IniciarComercioConUsuario(UserIndex, UserList(UserIndex).Flags.TargetUser)
    ...
    Case "/BOVEDA"
        ...
        If UserList(UserIndex).Flags.TargetNpc > 0 Then
          ...
          Call SendData(ToIndex, UserIndex, 0, "||Haz click izquierdo sobre el personaje." & FONTTYPE_INFO)
        End If
        Exit Sub
    '[/KEVIN]------------------------------------
    '[Alejo]
    ...
    Case "/ENLISTAR"
       ...
       Exit Sub
        ...
        'Salir del Cielo
       If UserList(UserIndex).Flags.TargetNpc = 0 Then
           Call SendData(ToIndex, UserIndex, 0, "||Primero tienes que seleccionar un personaje, hace click izquierdo sobre el." & FONTTYPE_INFO)
           Exit Sub
       End If
       ...
       End If
               ...
               Call WarpUserChar(UserIndex, Perdon.Map, Perdon.X, Perdon.Y, True)
    ...
    Case "/SALIRINFIERNO"
       ...
       If Npclist(UserList(UserIndex).Flags.TargetNpc).NPCtype <> 17 _
       Or UserList(UserIndex).Flags.Muerto <> 0 Then Exit Sub
       ...
       End If
               ...
               Call WarpUserChar(UserIndex, Volver.Map, Volver.X, Volver.Y, True)
...
Select Case UCase$(Left$(rdata, 2))
    ...
    Case "TI" 'Tirar item
        On Error GoTo errTirarOro
            ...
            Exit Sub
errTirarOro:
            Call LogError("ULISES:Tirar Oro Peta con User " & UserList(UserIndex).Name)
          ...
          If CI > 0 Then
                   If Npclist(CI).Flags.Domable > 0 Then
                        ...
                        If Npclist(CI).Flags.AttackedBy <> "" Then
                              Call SendData(ToIndex, UserIndex, 0, "||No puedes domar una criatura que está luchando con un jugador." & FONTTYPE_INFO)
                              Exit Sub
                        End If
...
Select Case UCase$(Left$(rdata, 9))
    ...
    Case "/RETIRAR " 'RETIRA ORO EN EL BANCO
         ...
         'Se asegura que el target es un npc
         If UserList(UserIndex).Flags.TargetNpc = 0 Then
              Call SendData(ToIndex, UserIndex, 0, "||Selecciona al Comerciante." & FONTTYPE_INFO)
              Exit Sub
         End If
         rdata = Right$(rdata, Len(rdata) - 9)
         If Npclist(UserList(UserIndex).Flags.TargetNpc).NPCtype <> NPCTYPE_BANQUERO _
...
Select Case UCase$(Left$(rdata, 11))
    Case "/DEPOSITAR " 'DEPOSITAR ORO EN EL BANCO
        ...
        'Se asegura que el target es un npc
        If UserList(UserIndex).Flags.TargetNpc = 0 Then
              Call SendData(ToIndex, UserIndex, 0, "||Haz click izquierdo sobre el Comerciante." & FONTTYPE_INFO)
              Exit Sub
        End If
        If Distancia(Npclist(UserList(UserIndex).Flags.TargetNpc).Pos, UserList(UserIndex).Pos) > 10 Then
                  Call SendData(ToIndex, UserIndex, 0, "||Estas demasiado lejos." & FONTTYPE_INFO)
        ...
        If CLng(val(rdata)) > 0 And CLng(val(rdata)) <= UserList(UserIndex).Stats.GLD Then
              ...
              Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & "Tenes " & UserList(UserIndex).Stats.Banco & " monedas de oro en tu cuenta." & "°" & Npclist(UserList(UserIndex).Flags.TargetNpc).Char.CharIndex & FONTTYPE_INFO)
        Else
              Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & " No tienes esa cantidad." & "°" & Npclist(UserList(UserIndex).Flags.TargetNpc).Char.CharIndex & FONTTYPE_INFO)
        End If
        Call SendUserStatsBox(val(UserIndex))
        Exit Sub
  Case "CLANDETAILS"
...
If UCase$(Left$(rdata, 8)) = "/CARCEL " Then
    ...
    If UCase$(Name) = "Neptuno" Then Exit Sub
