...
If UserList(UserIndex).Flags.PuedeAtacar = 1 Then
    ...
    If MapData(AttackPos.Map, AttackPos.X, AttackPos.Y).NpcIndex > 0 Then
        ...
        If Npclist(MapData(AttackPos.Map, AttackPos.X, AttackPos.Y).NpcIndex).Attackable Then
            ...
            If Npclist(MapData(AttackPos.Map, AttackPos.X, AttackPos.Y).NpcIndex).MaestroUser > 0 And _
               MapInfo(Npclist(MapData(AttackPos.Map, AttackPos.X, AttackPos.Y).NpcIndex).Pos.Map).Pk = False Then
                    Call SendData(ToIndex, UserIndex, 0, "||No puedes atacar mascotas en zonas seguras" & FONTTYPE_FIGHT)
                    Exit Sub
            End If
        ...
        Else
            Call SendData(ToIndex, UserIndex, 0, "||No puedes atacar a este NPC" & FONTTYPE_FIGHT)
        End If
...
If MapInfo(UserList(VictimIndex).Pos.Map).Pk = False Then
    Call SendData(ToIndex, AttackerIndex, 0, "||Esta es una zona segura, aqui no puedes atacar otros usuarios." & FONTTYPE_WARNING)
    PuedeAtacar = False
    Exit Function
End If

If MapData(UserList(VictimIndex).Pos.Map, UserList(VictimIndex).Pos.X, UserList(VictimIndex).Pos.Y).Trigger = 4 Then
    Call SendData(ToIndex, AttackerIndex, 0, "||No puedes pelear aqui." & FONTTYPE_WARNING)
    PuedeAtacar = False
    Exit Function
End If
...
If UserList(VictimIndex).Flags.Privilegios >= 1 Then
    SendData ToIndex, AttackerIndex, 0, "||¡¡No puedes atacar a los administradores del juego!! " & FONTTYPE_WARNING
    PuedeAtacar = False
    Exit Function
End If

If UserList(VictimIndex).Flags.Muerto = 1 Then
    SendData ToIndex, AttackerIndex, 0, "||No puedes atacar a un espiritu" & FONTTYPE_INFO
    PuedeAtacar = False
    Exit Function
End If

If UserList(AttackerIndex).Flags.Muerto = 1 Then
    SendData ToIndex, AttackerIndex, 0, "||No puedes atacar porque estas muerto" & FONTTYPE_INFO
    PuedeAtacar = False
    Exit Function
End If
...
If UserList(AttackerIndex).Flags.Seguro Then
        If Not Criminal(VictimIndex) Then
            If Not Criminal(AttackerIndex) Then
                Exit Function
            Else
                Call SendData(ToIndex, AttackerIndex, 0, "||No puedes atacar ciudadanos, para hacerlo debes desactivar el seguro apretando la tecla S" & FONTTYPE_FIGHT)
                Exit Function
            End If
        End If
End If
