...
If FileExist(DirDats & "\AGUAS.dat", vbArchive) = False Then
...
Leer.Initialize (DirDats & "\AGUAS.dat")
...
On Error GoTo Fallo
    ...
    ReDim NpcData(1 To NumNPCsHOST + 500) As NpcData
    ...
    For NPC = 1 To NumNPCsHOST
        NpcData(NPC + 500).name = Leer.GetValue("NPC" & (NPC + 500), "Name")
        NpcData(NPC + 500).Body = Val(Leer.GetValue("NPC" & (NPC + 500), "Body"))
        NpcData(NPC + 500).Head = Val(Leer.GetValue("NPC" & (NPC + 500), "Head"))
        NpcData(NPC + 500).Heading = Val(Leer.GetValue("NPC" & (NPC + 500), "Heading"))
        If LenB(NpcData(NPC + 500).name) <> 0 Then frmMain.lListado(2).AddItem NpcData(NPC + 500).name & " - #" & (NPC + 500)
    Next NPC
    Exit Sub
Fallo:
