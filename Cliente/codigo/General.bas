...
'Helios agrego condicion
If FileExist(App.Path & "\init\servers.txt", vbNormal) Then
    Dim intFile As Integer
    intFile = FreeFile
    Open App.Path & "\init\servers.txt" For Input As #intFile
    RawServersList = Input(LOF(intFile), #intFile)
    Close #intFile
End If
